...
BEGIN
  ...
  LOOP
    ...
    LOOP
      IF REGEXP_LIKE(l_hint, '^FULL.*KOKBF')  THEN
        DBMS_OUTPUT.PUT_LINE('q''['||REGEXP_REPLACE(REGEXP_REPLACE(l_hint, 'FULL', 'DYNAMIC_SAMPLING'),'\)', ' 4)') || ']'',');
        l_hint := NULL;
      else
        l_hint := NULL;
      END IF;
  ...
  DBMS_OUTPUT.PUT_LINE('validate    => FALSE,');
