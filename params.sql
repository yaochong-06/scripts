...
case when ksppinm = '_optim_peek_user_binds' and ksppstvl <> 'TRUE' then 'Recommended to set to default value <TRUE>, Peep through statistical information histogram binding variables'
     when ksppinm = '_undo_autotune' and ksppstvl = 'TRUE' then '_undo_autotune recommended to set to <FALSE>, default value is TRUE' 
...
case when name = 'open_cursors' and (value < 300 or value > 1000) then 'open_cursors recommend to set to more than <300>, 1000 is enough'
     when name = 'control_file_record_keep_time' and value <= 7 then 'control_file_record_keep_time recommend to set to <15>, 30 is enough' 
     ...
     when name = 'deferred_segment_creation' and value <> 'FALSE' then '<TRUE> will result in the failure to export an empty table'
